...
def integrator (h1,V,V2,V3):
    ...
    for t in np.arange(0,70000,h1):
        ...
        corr.append((V+V2)/2)
        #Euler integrator
        V=V+(f_V(V,m,h,n,I,r,Ca, h_TLT,h_THT, m_AHP) - g_GJ*(V - V2)+ noise*np.random.normal(0,1,1)*h1**(0.5))*h1 
        ...
        V2=V2+(f_V(V2,m2,h2,n2,I2,r2,Ca2, h_TLT2,h_THT2, m_AHP2) - g_GJ*(V2 - V) - g_GJ*(V2 - V3)+ noise*np.random.normal(0,1,1)*h1**(0.5))*h1 
        ...
        V3 = float(V3)
        l_Voltage3.append(V3)
        V3=V3+(f_V(V3,m3,h3,n3,I2,r3,Ca3, h_TLT3,h_THT3, m_AHP3) -g_GJ*(V3-V2)+ noise*np.random.normal(0,1,1)*h1**(0.5))*h1 
		#Y2=Y2+(f_V(Y2,m2,h2,n2,I2,r2,Ca2, h_TLT2,h_THT2, m_AHP2) + np.random.normal(0,1,1)*h1**(0.5))*h1
        m3=m3+f_m(V3,m3)*h1
        h3=h3+f_h(V3,h3)*h1
        n3=n3+f_n(V3,n3)*h1
        r3=r3+f_r(V3,r3)*h1
        m_AHP3= m_AHP3+f_m_AHP(Ca3, m_AHP3)*h1
        Ca3= Ca3+ f_Ca(I_TLT, Ca3, h_TLT3,h_THT3)*h1
        h_TLT3 =h_TLT3 + f_h_TLT(V3,h_TLT3)*h1
        h_THT3 =h_THT3 + f_h_THT(V3,h_THT3)*h1
        
        
        
    return [l_time, l_Voltage, l_Voltage2,l_Voltage3]
z=integrator(h1,V,V2,V3)
np.savetxt("0.004_1_triple.csv",np.array(z))
"""
# y-axis values
y1 = z[2]
...
plt.title('3 neurons {} and {} gwn'.format(g_GJ, noise))
 
# using the twinx() for creating another
# axes object for secondary y-Axis

ax.plot(z[0], y1, color = 'r', label="Neuron 2")
ax2 = ax.twinx()
ax2.plot(z[0], y2, color = 'b',label = "Neuron 3")  
ax3 = ax.twinx()
ax3.plot(z[0],y3,color='g',label = "Neuron 1")
...
#neural signals 1 and 2
s1=np.array(z[1])
s2=np.array(z[2])

#Apply the Hilbert transform to each signal
s1_hilbert = hilbert(s1)
s2_hilbert = hilbert(s2)

#Measure the phase difference between the two signals
phase_diff = np.angle(s1_hilbert) - np.angle(s2_hilbert)
plt.plot(z[0], phase_diff)
plt.show()
"""
