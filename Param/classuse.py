...
class Currents(cell_biophysics):
    ...
    def I_TLT(self):
        return g_TLT*(Currents.m_TLT_inf(self.V)**2)*self.h_TLT*(self.V-self.E_Ca(self.Ca))
        
    def I_THT(self):
        return g_THT*(self.m_THT_inf(self.V)**2)*self.h_THT*(self.V-Currents.E_Ca(self.Ca))
...
class HTC_cell(Currents):
    ...
    def integrator(self):
        h1 = 0.01 #ms
        duration = 10000 #ms
        V_tsd = np.zeros(duration/h1)
        time = np.zeros(duration/h1)
